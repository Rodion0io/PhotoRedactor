...
class CryptImage {
    ...
    public fun CryptTextInImg (pict: Bitmap, secretPic: Bitmap): Bitmap
    {
        for(x in 0 until  secretPic.width)
        {
            for (y in 0 until  secretPic.height)
            {
                var pixel = pict.getPixel(x, y)
                var pixleSecret = secretPic.getPixel(x, y)
                var red = Color.red(pixel)
                var green = Color.green(pixel)
                var blue = Color.blue(pixel)

                var newColor = replaceBits(red, green, blue,Color.red(pixleSecret))
                red = newColor[0]
                green = newColor[1]
                blue = newColor[2]
